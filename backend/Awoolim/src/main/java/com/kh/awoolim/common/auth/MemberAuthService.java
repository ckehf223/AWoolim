...
//	}
	...
	@Transactional
	public void registerMember( String email, String pw, String name, String phone, String birth,
			String gender) {
...
//		if (member == null) {
//			String pw = "office24";
//			registerMember(kakaoUser.getEmail(), pw, kakaoUser.getName(), kakaoUser.getPhone(), kakaoUser.getEmail(),
//					kakaoUser.getBirth(), kakaoUser.getGender());
...
//	}
	...
	public Member getNaverUser(String accessToken) {
		...
		if (response.getStatusCode() == HttpStatus.OK) {
			...
			Member member = new Member();
			member.setUserEmail((String) responseBodyObj.get("email"));
			member.setUserName((String) responseBodyObj.get("name"));
			member.setUserGender((String) responseBodyObj.get("gender"));
			member.setUserPhone(((String) responseBodyObj.get("mobile")).replace("-", ""));
...
//					String birthDate = dateFormat.format(birthDateStr);
					member.setUserBirth(birthDateStr);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public Member findOrCreateNaverUser(String code) {
		...
		Member member = memberMapper.findByEmail(naverUser.getUserEmail());
		if (member == null) {
			String generatedPw = "1234";
			registerMember(naverUser.getUserEmail(), generatedPw, naverUser.getUserName(), naverUser.getUserPhone(),
					 naverUser.getUserBirth(), naverUser.getUserGender());
			member = memberMapper.findByEmail(naverUser.getUserEmail());
		}
		return member;
	}

//	public String getGoogleLoginUrl() {
//		return "https://accounts.google.com/o/oauth2/auth?client_id=" + googleClientId + "&redirect_uri="
//				+ googleRedirectUri + "&response_type=code"
...
//				member.setUserEmail((String) emails.get(0).get("value"));
//			}
//
