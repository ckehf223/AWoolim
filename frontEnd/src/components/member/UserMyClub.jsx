...
const UserMyClub = () => {
  ...
  useEffect(() => {
    const getMyClub = async () => {
      try {
        const response = await instance.get('http://localhost:8080/api/club/readMyClub', {
  ...
  const onChangeSort = (e) => {
    ...
    const filteredApprClub = value === '전체' ? clubData.apprList : clubData.apprList.filter(club => club.category === value);
    const filteredDisaClub = value === '전체' ? clubData.disaList : clubData.disaList.filter(club => club.category === value);
                ...
                <>
                  ...
                  {disaClub.map((club) => (
                    <div className='UserMyClubBox' key={club.clubNo}>
                      ...
                      <div className='UserMyClubButtonArea'>
                        <button className='UserMyClubExitButton' onClick={() => {
                          openExitModal({
                            title: '신청 취소',
                            clubNo: club.clubNo,
                            clubTitle: club.clubTitle,
