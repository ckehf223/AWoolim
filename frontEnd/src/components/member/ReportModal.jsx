...
const ReportModal = ({ isOpen = false,
  ...
  const handleSendClick = async () => {
    if (content !== null && content !== '') {
      try {
        await instance.post('http://localhost:8080/api/report/register', {
          userId: userId,
          targetId: targetId,
          content: content,
          type: type
        ...
        <div className="modal-content">
          ...
          <ModalBody className='ReportModalBody'>
            ...
            <div className='ReportModalContentArea'>
              ...
              <input type="text" value={content} onChange={onChangeContent} placeholder='내용 입력을 입력하세요' readOnly={isSuccess} />
              <p className='ReportModalWarningMessage'>허위 신고시 제제 대상이 될 수 있습니다.</p>
            </div>
            {!isSuccess ? (<div className='ReportModalSendButtonArea'>
              <button className='ReportModalSendButton' onClick={handleSendClick}>신고</button>
            </div>) : (
              <p className='ReportModalSuccessMessage'>신고가 완료되었습니다.</p>)}
          </ModalBody>
        </div>
      </div>
    </Modal >
