...
const ClubRegister = () => {
  ...
  const onRegisterClub = () => {
    if (dayValue === '' || regularyType === '' || selectedButton === '' || selectedCity === ''
      || clubName === '' || !category === '' || age === '' || maxPerson === '') {
      ...
      try {
        ...
        instance.post('/api/club/register', formData, {
          headers: {
          ...
          if (response.data === 1) {
            ...
            alert('최대 개설 가능 모임은 3개 입니다.\n 기존 모임 삭제 후 이용 가능합니다.')
            nav('/', { replace: true });
          }
        }).catch(error => {
          console.log(error);
          alert('모임 등록 요청 중 오류가 발생하였습니다 \n 잠시 후 다시 시도해주세요');
        })
      } catch (error) {
        console.log(error);
        alert('모임 등록 요청 중 오류가 발생하였습니다 \n 잠시 후 다시 시도해주세요');
      ...
      <div className="ClubRegister">
        <div className="ClubRegisterWrap">
          ...
          <div className="ClubRegisterTopArea">
            <div className="ClubRegisterTopBox">
              <div className="fileBox">
                ...
                <input className="upload-name" value={fileInput} onChange={handleImageChange} placeholder="첨부파일" disabled />
                ...
                <button className="ClubRegisterImageDeleteButton" onClick={() => { onDeleteImage }}>삭제</button>
              </div>
              <div className="ClubRegisterNcBox">
                <div className="ClubRegisterNameBox">
                  <label htmlFor="ClubName">모임명</label>
                  <input type="text" name="ClubName" id="ClubName" value={clubName} onChange={changeClubName} />
                </div>
                <div className="ClubCategoryBox">
                  <label htmlFor="ClubCategory">카테고리</label>
                  <select value={category} onChange={changeCategory}>
                    <option defaultValue={""} value={""} defaultChecked>선택하세요</option>
              ...
              <div className="ClubRegisterChoiceSection">
                <div className="choiceBoxArea">
                  <div className="choiceBox">
                    ...
                    <input type={type} name="regularyType" value={dayValue} onChange={onSetInput} min={getTodayDate()} />
                  </div>
                  <div className="choiceBox">
                    <label htmlFor="MaxPerson">정원</label>
                    <input type="text" name="MaxPerson" id="MaxPerson" value={maxPerson} onChange={changeMaxPerson} placeholder="숫자만 입력" />
                  </div>
                  <div className="choiceBox">
                    <label htmlFor="ClubGender">모집 성별</label>
                    <select name="ClubGender" id="ClubGender" value={gender} onChange={changeGender}>
          ...
          <div className="ClubRegisterButtonArea">
            <button className="registerButton" onClick={() => { onRegisterClub() }}>등록</button>
